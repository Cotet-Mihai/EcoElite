...
import { businessEmail, clientEmail } from "@/utils/email-template";
import type { RequestPayload, RequestResult } from "@/utils/request";
...
const FROM_BUSINESS = "Formular Eco Elite <no-reply@ecoelite.ro>";
const FROM_CLIENT = "Eco Elite <no-reply@ecoelite.ro>";
...
export async function sendContact(payload: RequestPayload): Promise<RequestResult> {
    ...
    if (!apiKey) {
        ...
        return {
            ok: false,
            error: `Trimiterea nu e disponibilă momentan. Sună-ne la ${COMPANY.phone} și rezolvăm imediat.`,
        };
    }
    ...
    try {
        const toBusiness = await resend.emails.send({
            ...
            subject: `Solicitare de pe site — ${payload.name}`,
        ...
        if (toBusiness.error) {
            ...
            return {
                ok: false,
                error: `Nu am putut trimite mesajul. Sună-ne la ${COMPANY.phone} și rezolvăm imediat.`,
            };
        }

        // Confirmarea către client e secundară: dacă pică, cererea tot a ajuns la
        // Eco Elite, deci nu spunem vizitatorului că a eșuat ceva.
        const toClient = await resend.emails.send({
            from: FROM_CLIENT,
            to: payload.email,
            subject: "Am primit mesajul tău · Eco Elite",
